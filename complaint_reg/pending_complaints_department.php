...
<body>
    <center>
    <h5 align="center"><u>LIST OF PENDING COMPLAINTS
<?php
if (!empty($_GET["compType"])) {
    echo ' "' . ((int)$_GET["compType"]) . '"';
}
?>
 FOR YOUR DEPARTMENT AS ON Date : "<?php echo (date('Y-m-d')); ?>"</u><br></h5>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($selectedType)) {
        $query = "SELECT DISTINCT CT.CompNo AS CompNo, CT.Time AS CompTime, CT.CompDate AS CompDate, EMS.loc_desc AS Plant, CT.Location AS Location,CT.ContactNo AS ContactNo,
